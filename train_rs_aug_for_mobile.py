...
def main(user_args):
    ...
    for epoch in range(args.max_epoch):    
        for batch_idx, (image_input) in enumerate(train_loader, 0):
            scale_1 = np.random.uniform(0, 0.3)
            cropper = T.RandomResizedCrop(size=(400, 400), scale=(0.7, 1))
            rotater = T.RandomRotation(degrees=(-30, 30))
            jitter = T.ColorJitter(brightness=scale_1, contrast=scale_1, saturation=scale_1, hue=0)
            ...
            inputs   = torch.cat((rs_image_input, rs_stamped_image), dim=0)
            preds   = detector(inputs)
            loss_acc = celoss(preds, targets)
            
            ## augmentation accuracy
            combine_inputs = torch.cat((image_input, stamped_image), dim=0)
            new_image = combine_inputs
            if np.random.rand()<=args.aug_prb: new_image = jitter(new_image)
            if np.random.rand()<=args.aug_prb: new_image = cropper(new_image)
            if np.random.rand()<=args.aug_prb: new_image = blurrer(new_image)
            if np.random.rand()<=args.aug_prb: new_image = rotater(new_image)
            if np.random.rand()<=args.aug_prb: new_image = perspective_transformer(new_image)
            if np.random.rand()<=args.aug_prb: new_image = jpeg(new_image)
            combine_new_inputs = torch.clamp(new_image, 0, 1)
            rs_combine_new_inputs = combine_new_inputs + torch.randn_like(combine_new_inputs)*args.std   
            rs_aug_preds    = detector(rs_combine_new_inputs)
            loss_acc_aug    = celoss(rs_aug_preds, targets)
            ...
            weight_aug = max(0, min(args.aug_w*(global_step-args.aug_start)/(args.aug_end-args.aug_start), 1))

            loss_all = args.img_w*image_loss + args.lps_w*lpips_loss + args.rs_w*loss_acc + weight_aug*loss_acc_aug
            ...
            if global_step % args.log_interval == 0:
                name_space = ['image_loss:' , 'lpips_loss:',  'rgb_image_loss:', 'loss_acc:', 'loss_acc_aug:']
                losses_log = [image_loss.item(), lpips_loss.item(),   rgb_image_loss.item(),loss_acc.item(), loss_acc_aug.item()]
            ...
            if global_step % 20 == 0:
                ...
                writer.add_scalar('loss/loss_acc_aug', loss_acc_aug, global_step)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--aug_start', type=int, default=1000)
