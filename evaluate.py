...
def acc_calculate(detector, img_non_stmp, img_stmp, ones, zeros, std):
    ...
    preds  = detector(img_non_stmp)
    pred_ones_ori = torch.argmax(preds, dim=1)
    
    preds = detector(img_stmp)
...
#     values = [ori_acc, rtt_acc, crp_acc, wrp_acc, blr_acc, jtr_acc]
    ...
    for ind, eps in enumerate(eps_list):
        ...
        pgd         = PGD_DEC(detector, eps=eps, alpha=alpha_list[ind], steps=40, random_start=True)        
        for batch_idx, (image_input ) in enumerate(test_loader, 0):
            ...
            ## FGSM perturbation
#             img_non_stmp = fgsm_attack(detector, image_input, ones_labels, eps) 
#             img_stmp     = fgsm_attack(detector, stamped_image, zeros_labels, eps) 
            ...
            ## PGD perturbation
#             img_non_stmp = pgd_attack(detector, image_input, ones_labels, eps, alpha_list[ind]) 
#             img_stmp     = pgd_attack(detector, stamped_image, zeros_labels, eps, alpha_list[ind]) 
