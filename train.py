...
from evaluate import evaluate, acc_calculate
...
def main(user_args):
    ...
    val_loader   = DataLoader(val_dataset,   batch_size=args.batch_size, num_workers=8, shuffle=False, pin_memory=True, drop_last=True) ## this is just for monitering the performance during training. Did not use validation performance for any training setting.
...
#     celoss  = torch.nn.MSELoss()
    ...
    ## initialization
    global_step = 0
#     ones = torch.ones(args.batch_size).int().cuda()
#     zeros = torch.zeros(args.batch_size).int().cuda()
    ...
    for epoch in range(args.max_epoch):    
        for batch_idx, (image_input) in enumerate(train_loader, 0):
            ...
            imputs   = torch.cat((image_input, stamped_image), dim=0)
            ...
            if args.aug:
                ...
                aug_inputs   = torch.cat((img_non_stmp, img_stmp), dim=0)
                aug_preds   = detector(aug_inputs)
                loss_acc_aug = celoss(aug_preds, targets)
                loss_acc = loss_acc + loss_acc_aug
            ...
            if global_step % args.log_interval == 0:
                ...
                writer.add_scalar('val/apgd_nonstamp_acc', mix_nonstamp_acc, global_step)
...
def validate(encoder, detector, val_loader, args):
    ...
    with torch.no_grad():
        ...
        auto_attack = AutoAttack(detector, norm='Linf', eps=16/255, version='standard')
        ...
        apgd_nonstamp_acc = apgd_nonstamp_acc/(i1+1)
        apgd_stamp_acc    = apgd_stamp_acc/(i1+1)
        apgd_acc          = apgd_acc/(i1+1)
...
if __name__ == '__main__':
    ...
    if not os.path.exists(dataset_folder):
        print('download your mirflickr dataset in this folder. ')
